...
def create_models(metric, fold):
    meta_data = pd.read_pickle('src/results/TCA/process/tca.pkl')
    source_projects = meta_data.source.unique()
    pseudo_source_projects = meta_data.target.unique()

    median_data = pd.read_pickle('src/esults/attributes/projects_median.pkl')

    cluster_data_loc = 'src/results/median_data_1/level_2/fold_' + str(fold)
    ...
    for src in pseudo_source_projects:
        ...
        data_vectors[src] = median_data.loc[src].values.tolist()
    ...
    final_result_df.to_csv('src/results/median_data_1/level_2/fold_' + str(fold) + '/predicted_source_process.csv')
    return None
